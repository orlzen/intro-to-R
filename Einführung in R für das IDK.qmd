...
```{r}
#| echo: FALSE
# knitr::purl(input = "Einführung in R für das IDK.qmd", output = "Einführung in R für das IDK - Code.R", documentation = 0)
```
...
Die Daten können mit dem untenstehenden Button "Daten herunterladen" heruntergeladen werden. Bitte entpackt das .zip und speichert die Daten ab, idealerweise in einem Ordner namens "Daten", welcher sich in einem Hauptordner befindet, wo das Skript "Einführung in R für das IDK - Codes.R" abgelegt ist.
...
-   Am wichtigsten:
    ...
    -   Geladene Daten & Objekte in der Umgebung ("Environment"; oben rechts)
...
-   Achtung: Mit jedem benutzten Paket entstehen Abhängigkeiten (einige Pakete benötigen wiederum andere Pakete), im Extremfall funktioniert ein Paket plötzlich nicht mehr oder der Code, den man beim letzten Mal benutzt hat, funktioniert anders (für diese spezifische Herausforderungen bei der Reproduzierbarkeit gibt es wiederum eigene Lösungen...).
-   Pakete, die auf CRAN ("The Comprehensive R Archive Network", <https://cran.r-project.org/web/packages/available_packages_by_name.html>) sind, haben eine gewisse Qualitätskontrolle durchlaufen. Es lassen sich aber auch individuelle Pakete installieren, die nicht auf CRAN sind (jede:r kann / darf ein R-Paket schreiben).
...
![Neues Projekt eröffnen](images/Screenshot%20(39).png){#fig-new-project alt="Neues Projekt eröffnen"}
...
-   Allgemein gültige Pfade mit dem Paket {here} -\> laden am Anfang mit `library(here)`
    -   R weiss dann "`here() starts at C:/Users/bibs/OneDrive - ZHAW/R/Einführung in R für das IDK`"
...
R kann verschiedene Daten(sätze) *gleichzeitig laden* (oben rechts in "Environment"; @fig-environment-list und @fig-environment-grid, je nach Darstellung), diese können *verschiedene Typen* sein:
...
[^1]: Erweiterungen des klassischen data frames aus {base} R sind "tibbles" im tidyverse-Universum (siehe @sec-tidyverse) oder "data.tables" bei Benutzung vom Paket {data.table} (siehe <https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html>)
...
-   Befehle müssen deshalb die zu benutzenden Objekte *eindeutig identifizieren*
    ...
    -   Z.B. weist `iris$Sepal.Width` hin auf die Variable "Sepal.Width" im Datensatz "iris", `ToothGrowth$len` auf Variable "len" im Datensatz "ToothGrowth" etc.
-   Daten können auch angeschaut werden, entweder mit `View()` oder durch klicken auf das Objekt im Fenster "Environment"
-   In einer Analyse können Variablen aus verschiedenen Datensätzen kombiniert werden, z.B. `(mean(iris$Sepal.Width)) + (median(ToothGrowth$len))`
...
-   Spezielle Formate brauchen ggf. spezielle Pakete, wie {haven} [@haven] für den Import von SPSS- oder STATA-Daten
    -   **Achtung**: R geht mit Labels ganz anders um, können ggf. nicht importiert werden (siehe @sec-kategoriale-variablen)!
...
## Kategoriale / nominale Variablen & labels {#sec-kategoriale-variablen}

-   Auch in R werden kategoriale Variablen als Nummer (integers) gespeichert mit einem zugewiesenen "level"
-   R zeigt aber normalerweise nur die "levels" an, nicht die dahinterliegenden integers
-   Labels für numerisch codierte kategoriale Variablen sind in R eher untypisch
-   Häufiger sind die Variablen direkt codiert, als "männlich" oder "weiblich" anstatt "0 = männlich" und "1 = weiblich", d.h. die Variablen werden als Faktorvariablen gespeichert
...
-   vorschlag von bibs: Für einfache, schnelle, explorative Abbildungen die einfachen Abbildungen aus dem {graphics}-Paket nutzen, für schönere, individualiserte (z.B. für Publikationen) eher {ggplot2}.
...
-   **Achtung**: Weil die Funktion `abline()` getrennt von der Abbildung erstellt wird, wird sie einfach immer bei der letzten Grafik eingefügt. Wenn ich zwischen dem Histogramm und der Linie eine andere Abbildung erstellt habe, wird sie einfach dort eingefügt!
...
-   Literatur hier: @wickham2016a oder hier: <https://ggplot2-book.org/index.html>
-   Grundsätzliche Idee: Abbildung erstellen in Ebenen
    -   Zuunterst: Daten
    -   Nächste Ebene: Was soll wo hin (X- und Y-Achse, ggf. Füllung und Farbe)
...
-   "Learning Statistics with R", ein Manuskript für Psychologiestudierende von @navarro. Gut für die basics, ohne Verweise auf das Tidyverse (alles in base R)

-   Verschiedene (technische) Handbücher zu R: <https://rstudio.github.io/r-manuals/>

-   {chatgpt}: Es gibt ein Paket zu ChatGPT [@chatgpt-2], welches bei Code-Fragen helfen kann

    -   Mehr Infos unter <https://github.com/jcrodriguez1989/chatgpt>

    -   Installieren und Laden wie alle anderen Pakete (@sec-pakete)

    -   Nutzung über ein visuelles Interface oben unter "Addins" (@fig-addin-chatgpt)

    -   ChatGPT kann einen Code erstellen, wenn man der AI sagt, was man braucht oder umgekehrt kann umgekehrt einen Code erklären, den man der AI eingibt

    -   Man braucht einen API key (bekommt man einfach über den Zugang zu ChatGPT), welcher in RStudio hinterlegt werden muss: `Sys.setenv(OPENAI_API_KEY = "XX-XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")`

![Addin "ChatGPT"](images/Screenshot%20(48).png){#fig-addin-chatgpt}
...
-   Perfekte Zusammenarbeit mit Zotero!
...
-   Das tidyverse ist das ganze "Universum", zusammengesetzt aus verschiedenen Paketen. Je nach Bedürfnis werden mit `library(tidyverse)` gleich alle Pakete oder ansonsten nur einzelne geladen.

    -   {ggplot2}: @ggplot2-2, Daten visualisieren
...
-   Nutzung:
    ...
    -   `library(ggplot2)`: Lädt nur Paket "ggplot2"
