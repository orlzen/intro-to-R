...
-   Deskriptive Analysen, alltägliche Anwendungen für uns
    -   Umgang mit missing values 
        -   ("MISSING VALUES S1 (99999997).")
    -   Recodierungen von Variablen in neue Variablen
        -   RECODE S9 (2 = 1.5) (3 = 2) (4 = 2.5) (5 = 3) (6 = 3.5) (7 = 4) (8 = 4.5) (9 = 5) (10 = 5.5) (11 = 6) INTO pol_note_rec.
        -   EXECUTE.
        -   VARIABLE LABELS pol_note_rec "Benotung Polizei - recodiert".
        -   VALUE LABELS pol_note_rec 0 "Unsicher" 1 "Sicher".
        -   oder
        -   IF (Q4_06 = 1) cybercrime_was_identitaet = 0.
        -   IF (CODERESP = 18695) cybercrime_was_identitaet = 1.
        -   IF (CODERESP = 27740) cybercrime_was_identitaet = 1.
        -   IF (CODERESP = 9974) cybercrime_was_identitaet = 1.
        -   IF (CODERESP = 38720) cybercrime_was_identitaet = 1.
        -   IF (CODERESP = 17895) cybercrime_was_identitaet = 1.
    -   Recodierung nur wenn... ("do if" in SPSS)
    -   Neuberechnung von neuen Variablen `(dplyr::mutate?)`
        -   COMPUTE sex_gewichtung = SEX.
        -   EXECUTE.
        -   IF (sex_gewichtung = 3 AND sex_SRPH = "Frau") sex_gewichtung = 2.
        -   IF (sex_gewichtung = 3 AND sex_SRPH = "Mann") sex_gewichtung = 1.
        -   EXECUTE.
        -   VALUE LABELS sex_gewichtung 1 "männlich" 2 "weiblich".
        -   VARIABLE LABELS sex_gewichtung 'Geschlecht zur Gewichtung (Kombination aus Befragungs- und Adressdaten, anderes durch SRPH ersetzt)'.
    -   Labels
        -   Variablenlabels
        -   Value labels (möglich?)
-   Umgang mit Strings?
-   Objekte nicht nur numerisch zuweisen, sondern auch mit strings und logisch
-   Matrizen haben immer gleichen Datentyp in Spalte, data frame können verschiedene Daten haben (numerisch, string, logisch, faktoren)
-   Datentyp Faktoren
-   Lists are collections of objects, which can be vectors, matrices, data frames
-   Nested data?
-   Mehr Grundsätzliches am Anfang, weniger Fortgeschrittenes
-   Datensatz "USArrests" laden
    -   Daten sind links beschriftet ohne Variable
...
-   Einzelen Spalte/Variable auswählen mit Variablenname: `m[, "Var1"]` -> Spalte mit Namen "Var1", keine Zeile/Beobachtung ausgewählt
-   Alternative für Spalten/Variablen, wenn das Objekt ein data frame ist: `df$Var1` -\> Spalte/Variable genannt "Var1" ausgewählt
...
mean(iris[, 1]) # 1. Spalte
mean(iris[,"Sepal.Length"]) # Spalte mit Name "Sepal.Length"
mean(iris$Sepal.Length) # Spalte mit Name "Sepal.Length"
```

-   Analog kann auch auf Elemente von anderen Objekte zugegriffen werden, z.B. auf Elemente eines Outputs einer Operation, die als Listen gespeichert werden:
...
-   Keine Labels für numerisch codierte kategoriale Variabeln (kategoriale Variablen als Faktor-Variablen speichern!)
-   Umgang mit fehlenden Werten, keine individuelle Setzung von "missing" für versch. numerische Werte möglich
-   Gewichtung von Daten?
-   Pakete können umgeschrieben werden, Code kann sich ändern
-   Einfache Häufigkeits- und Kreuztabellen mit Prozenten sind nicht gleich schnell wie in SPSS
...
-   Eigene Syntaxstruktur[^1], aufbauend / hintereinander verknüpft, weniger verschachtelt als base R (wo man beim lesen mehrerer Funktionen "von innen" anfangen muss):
...
[^1]: Es gibt durchaus Stimmen, die sich gegenüber Tidyverse kritisch äussern mit dem Argument, es sei unnötig, mache alles komplizierter, und mit Base R sei auch alles bereits möglich, was das Tidyverse kann.
