...
-   Eigene Syntaxstruktur, aufbauend:
    -   Welche Daten sollen benutzt werden?
    -   Was soll damit gemacht werden?
    -   Verknüpfung mit dem "Pipe"-Operator (`|>` (früher `%>%`))
...
# Erstelle neue Variable "mean" in den neuen Daten "iris2"
iris2 <- iris |> # Nimm die Daten "iris" und schreibe eine neues Objekt "iris2"
  mutate(sepallength_width = (Sepal.Length + Sepal.Width)) |>  # Berechne die Summe aus "Sepal.Length" und "Sepal.Width" für jeden Fall und nenne die neue Variable "sepallength_width"
  group_by(Species) |> # Gruppiere die Daten nach "Species"
  mutate(mean = mean(sepallength_width)) # Berechne (für die gruppierten Daten!) den Mittelwert von "sepallength_width" und nenne diese neue Variable "mean"
```
