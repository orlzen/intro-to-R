...
csl: zhaw-soziale-arbeit-zitieren-und-belegen.csl
css: styles.css
---
...
Diese Übersicht wird laufend ergänzt. Bitte melden bei [bibs\@zhaw.ch](mailto:bibs@zhaw.ch), wenn etwas falsch oder unverständlich ist, fehlt oder ergänzt werden sollte oder direkt als issue im öffentlichen [github repository](https://github.com/orlzen/intro-to-R).
:::

::: callout-tip
Die Code-Beispiele können aus diesem html-Dokument direkt kopiert werden, indem man oben rechts im Code-Abschnitt auf das Notizbrett klickt.
:::
...
library(survey) # Für Arbeit mit Umfragedaten, spezifisch Gewichtung
```
...
Damit die Code-Beispiele in dieser Einleitung problemlos funktionieren, bitte die folgenden Schritte ausführen:
...
-   Siehe hier <https://rstats.wtf/projects>

::: callout-important
Absolute Pfade à la "`C:\Users\bibs\OneDrive - ZHAW\R\Einführung in R für das IDK`" sind **verpönt**, weil nicht übertragbar auf anderen Geräte / für andere Benutzer.
:::

-   Sobald Code mit jemand anderem (oder auch nur einem anderen eigenen Computer mit anderer Ordnerstruktur) geteilt werden soll: Allgemein gültige Pfade benutzen mit dem Paket {here} [@here] -\> laden am Anfang mit `library(here)`
    -   R arbeitet dann aus einem designierten Projektordner heraus, der irgendwo abgelegt werden kann (siehe <https://rstats.wtf/practice-safe-paths>.
...
## Fehlende Werte {#sec-fehlende-werte}

-   `NA` = fehlende Werte.
-   `NaN` = "not a number" (bspw. bei Rechnung `0 / 0`).
-   R ist empfindlicher gegenüber fehlenden Werten als z.B. SPSS, siehe Beispiel unten für eine Berechnung eines Mittelwertes oder @sec-korrelation-na für Korrelationen.
...
-   Bestimmten Wert als `NA` codieren: `df$Var1 <- na_if(df$Var1, Wert)`
-   Im Gegensatz zu SPSS können in R nicht verschiedene numerische Werte als "missing" codiert werden.
-   Weiterführendes Beispiel zur Kontrolle von fehlenden Werten und der Möglichkeit, fehlende Werte zu durch "imputatation" zu ergänzen: @feliz2023
...
-   Weiteres zur Auswahl von einzelnen Spalten ("subsetting"), mit Code-Beispielen für *base R* und das *Tidyverse* (@sec-tidyverse), siehe @brunner2023
...
-   Zur Nutzung von labels mit den Paketen {labelled} [@labelled] und {sjlabelled} [@sjlabelled], siehe der Blog-Post von @pileggi2020

::: callout-caution
Wenn Daten in R gelabelt und dann mit {haven} exportiert werden sollen in ein SPSS-Format: Die beiden Pakete können Konflikte erzeugen, siehe hier (zuunterst, Abschnitt "Interaction with 'haven'"): <https://cran.r-project.org/web/packages/expss/vignettes/labels-support.html>
:::
...
-   Einfachster Code für eine Korrelationsanalyse: `cor(x, y)`
-   Default-Korrelation wird mit "pearson" berechnet (siehe `?cor()`).

```{r}
cor(iris$Sepal.Length, iris$Petal.Length)
```

-   Andere Korrelationsmethoden können definiert werden mit `cor(x, y, method = "kendall")` oder`cor(x, y, method = "spearman")`.

```{r}
cor(iris$Sepal.Length, iris$Petal.Length, method = "spearman")
```

-   Es kann auch gleich eine Korrelationsmatrix für mehrere Variablen berechnet werden mit `cor(df)` (anstatt zwei Variablen wird einfach der ganze data frame an die Funktion übergeben).
    -   **Achtung**: Das geht natürlich nur für metrische Variablen, nicht-metrische Variablen müssen deshalb entweder ausgeschlossen werden (z.B. indem ein neuer Datensatz erstellt wird) oder die Auswahl der Variablen muss eingeschränkt werden, wie unten.
    -   Für eine bessere Übersicht werden die Korrelationskoeffizienten gerundet auf zwei Nachkommastellen mit `round(..., digits = 2)`, wobei die Funktion zur Korrelation (`cor()`) gleich in die Rundungsfunktion `round()` eingebunden wird.

```{r}
# Auswahl des ganzen iris-Datensatz, Spalten 1 bis 4
round(cor(iris[, 1:4]), 
  digits = 2 # Gerundet auf zwei Dezimalstellen
)
```

-   Für die Visualisierung von Korrelationen mit base R mit einem Streudiagramm siehe @sec-streudiagramm-base (2 Variablen), @sec-pairs-base (Matrix mehrerer Variablen) oder @fig-iris-sepal-scatterplot mit {ggplot2}.

### Fehlende Werte (`NAs`) in Korrelationen {#sec-korrelation-na}

-   **Achtung** bei `NAs` in den Daten (siehe @sec-fehlende-werte)!
-   Die Variablen `car_sales$length` und `car_sales$curb_wgt` haben einige fehlende Werte.

```{r}
sum(is.na(car_sales$length))

sum(is.na(car_sales$curb_wgt))
```

-   Die Korrelation kann so mit diesen Daten nicht korrekt berechnet werden, wenn die `NAs` nicht ausgeschlossen werden.

```{r}
cor(car_sales$length, car_sales$curb_wgt)
```

-   Um die Korrelation nur mit den Fällen zu berechnen, welche keine `NAs` beinhalten, muss der Zusatz `(..., use = "complete.obs)"` verwendet werden.

```{r}
cor(car_sales$length, car_sales$curb_wgt, use = "complete.obs")
```
...
-   Basisfunktion: `lm(Zielvariable ~ Prädiktor1 + Prädiktor2 + Prädiktor3, data = Datensatz)`
-   Wird nur die Regression ausgeführt, erhält man als Resultat die eingegeben Formel (`Call`) und die Koeffizienten (`Coefficients`)
...
::: callout-tip
Das Modell der Regression in ein Objekt übergeben und Abspeichern mit `<-`.
:::
...
-   Mit `summary(...)` des gespeicherten Regressionsmodells erhält man detailliertere Resultate des berechneten Modells.

```{r}
summary(fit_iris)
```

-   Falls nicht extra ein Objekt gespeichert werden soll, aber trotzdem das detaillierte Modell interessiert, einfach beides kombinieren in einem Befehl: `summary(lm())`.
...
-   Interaktionsterme werden kombiniert mit `:`, z.B. `lm(Zielvariable ~ Prädiktor1 + Prädiktor2:Prädiktor3, data = Datensatz)`
...
-   Wird das Modell als Objekt gespeichert, können mit der `plot()`-Funktion verschiedene Elemente des Modells visualisiert werden (z.B. `plot(fit_iris, which = 1)` für einen Residuenplot), siehe @sec-analyse-objekte.
    -   Es sind sechs verschiedene Plots möglich aus einem Regressionsmodell, die mit `plot(..., which = x)` ausgewählt werden können.

```{r}
plot(fit_iris, 
# Auswahl des ersten Plots      
     which = 1)
```

### Logistische Regression

-   Basisfunktion `glm()` mit Zusatzoption `(..., family = "binomial")`
-   Hier als Beispiel mit den "Churn"-Daten und der Variable "Churn" als Zielvariable. Diese hat zwei Ausprägungen, da sie eine "logische" Variable ist (siehe `class(Churn$Churn)`) hat sie die Ausprägungen `TRUE` und `FALSE`.

```{r}
fit_churn_log <- glm(Churn ~ DayMins + EveMins + NightMins,
                    data = Churn,
                    family = "binomial")

summary(fit_churn_log)
```

# Abbildungen

-   Eine grosse Stärke von R ist die Möglichkeit, Daten relativ einfach schnell, und mit etwas mehr Aufwand publikationsbereit, zu erstellen.
...
-   Für eine Übersicht über mögliche Visualisierung in R, aufgetrennt nach Typ der Abbildung und unterschieden zwischen base R und {ggplot}, siehe <https://r-charts.com/>
...
## Streudiagramm {#sec-streudiagramm-base}

-   Metrische Variablen gegeneinander
-   Dritte Variable ("Species") als Farbe
-   Keine Legende standardmässig, muss manuell erstellt werden und ist nicht sehr intuitiv (weil zwei getrennte Objekte erstellt werden ("plot" und "legend"), müssen die Farben manuell aufeinander abgestimmt werden!). Hier ist ggplot2 (@sec-ggplot) schon einfacher.
...
-   Die *richtige* Stärke zur Datenvisualisierung in R lieght in der Benutzung des Pakets {ggplot2} [@ggplot2-4]. Dieses ist Teil des "Tidyverse"-Universum (siehe unten @sec-tidyverse) und benutzt entsprechend die gleiche Syntaxlogik wie andere "Tidyverse"-Pakete.
...
-   Literatur hier:
    ...
    -   @rapp2023 (online hier: <https://albert-rapp.de/posts/ggplot2-tips/18_ultimate_guide/18_ultimate_guide>)
    -   @chang
    -   *Fehler, die man nicht machen soll* in der Datenvisualisierung, mit Code für R: @c.li2023\
-   **Viele** Einstellungsmöglichkeiten bei Abbildungen, siehe z.B. @fig-iris-sepallength-histogram-ggplot (Histogramm mit Standardeinstellungen & vertikaler Linie beim Mittelwert der X-Achse), @fig-iris-sepallength-histogram-ggplot-facet (Histogramm mit "faceting", Aufteilung der Abbildung in bestehende Kategorien), @fig-iris-sepallength-histogram-ggplot-identity (Histogramm mit Füllung je nach Kategorie und leichter Transparenz (`alpha`)) oder @fig-iris-sepal-scatterplot (Streudiagramm mit Regressionsgeraden, aufgeteilt nach Kategorie).
...
ggplot(iris) +                                     # Daten 
    ...
    labs(                                          # Labels 
        ...
        y = "Häufigkeit") +                        # Titel
  geom_vline(xintercept = mean(iris$Sepal.Length), # Vertikale Linie bei Mittelwert
                                                   # von iris$Sepal.Length
             linetype = "dashed",                  # Gestrichelte Linie
             color = "red")                        # Farbe der Linie rot
```
...
-   @fig-iris-scatter:
    -   Füllfarbe der Punkte in der ZHAW-Farbe im HEX-Code (`#0064A6`), Punkte vergrössert (für die Umwandlung eines CMYK in Hex, z.B.: <https://colordesigner.io/convert/cmyktohex>)
...
ggplot(iris) +
  ...
  annotate("text",
           x = 1,
           y = 7.5,
           label = "Hier sind die grössten Blumen") +
  geom_curve(aes(x = 1,
                 y = 7.2,
                 xend = 1.7,
                 yend = 5.7),
...
# Resultate von Analysen als Objekte {#sec-analyse-objekte}
...
# Spezialfall gewichtete Umfragedaten

-   Am IDK arbeiten wir häufig mit gewichteten Umfragedaten. Die Gewichtung von Daten ist nicht in base R möglich.
-   Dazu kann z.B. das {survey}-Paket benutzt werden von @survey.
    -   Die Anwendung ist jedoch einiges komplizierter als bspw. in SPSS.
    -   Eine Anleitung gibt es z.B. bei <https://zacharylhertz.github.io/posts/2021/06/survey-package>.
-   Alternative: {srvyr} von @srvyr: Benutzt Funktionen aus dem {survey}-Paket, kann aber "pipes" (`|>`) und andere Funktionen aus dem Tidyverse (siehe @sec-tidyverse) benutzen. Für einen Vergleich von {survey} und {srvyr} siehe hier: <https://cran.r-project.org/web/packages/srvyr/vignettes/srvyr-vs-survey.html>.
...
# Here, my data is 'df' and weights are "var_gewicht".
svy.dat <- svydesign(ids =~ 1, 
                     data = df,
                     weights = df$var_gewicht)
```
...
-   Ein umfassendes Einführungswerk auf Deutsch zu R und RStudio, welches relativ ähnliche Themen behandelt wie diese Einführung gibt es von @großeschlarmann2023a
...
-   Open source Lehrbuch zum ganzen Tidyverse: [@wickham2016b] (Hadley Wickham ist Chief Scientist bei Posit, der Firma, die RStudio betreibt)

-   Blog-Post zur Datenmanipulation mit {dplyr}: @soetewey2023
...
-   Ist in RStudio integriert (oben rechts "Git"), braucht kein weiteres Programm (evtl. unter "Tools - Global Options - Git/SVN" den Pfad zur git-Installation anpassen ("Git executable"), meistens C:/Program Files/Git/bin/git.exe)
...
-   Verknüpfung von RStudio und GitHub mit einem Personal Access Token (PAT) wird hier beschrieben: <https://rfortherestofus.com/2021/02/how-to-use-git-github-with-r/> (unter "Connect RStudio and GitHub")
